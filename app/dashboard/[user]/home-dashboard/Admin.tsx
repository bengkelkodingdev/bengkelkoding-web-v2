"use client";
import { getAdminDashboard } from "@/app/api/admin/dashboard";
...
const HomeDashboardAdmin = () => {
  ...
  const [dashboardData, setDasboardData] = useState({
    ...
    active_classrooms_count: 0,
    available_tokens_count: 0,
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Get Dashboard Data
        const responseDashboardData = await getAdminDashboard();
        setDasboardData(responseDashboardData.data);
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-3 2xl:gap-4">
        ...
        <Link
          href={`${userRole.role}/pengguna/mahasiswa`}
          className={`flex items-center gap-4 p-6 lg:p-8 bg-gradient-to-r text-white rounded-lg hover:shadow-[rgba(0,_0,_0,_0.1)_0px_6px_10px] focus:ring-4 transition-all ease-out duration-200 from-blue-700 to-blue-600 focus:ring-blue-100`}
        >
          ...
          <div>
            <strong className="text-2xl lg:text-3xl 2xl:text-4xl">
              {dashboardData.active_students_count}
            </strong>
            <p className="text-neutral5">Jumlah Mahasiswa</p>
          </div>
        </Link>
        {/* Active Classroom Count */}
        <Link
          href={`${userRole.role}/kelas`}
          className={`flex items-center gap-4 p-6 lg:p-8 bg-gradient-to-r text-white rounded-lg hover:shadow-[rgba(0,_0,_0,_0.1)_0px_6px_10px] focus:ring-4 transition-all ease-out duration-200 from-red-700 to-red-600 focus:ring-red-100`}
        >
          <div className="w-max p-4 rounded-full bg-white fill-white bg-opacity-10">
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div>
            <strong className="text-2xl lg:text-3xl 2xl:text-4xl">
              {dashboardData.active_classrooms_count}
            </strong>
            <p className="text-neutral5">Kelas Aktif</p>
          </div>
        </Link>
        {/* Total Feedback Count */}
        <Link
          href={`${userRole.role}/feedback`}
          className={`flex items-center gap-4 p-6 lg:p-8 bg-gradient-to-r text-white rounded-lg hover:shadow-[rgba(0,_0,_0,_0.1)_0px_6px_10px] focus:ring-4 transition-all ease-out duration-200 from-orange-700 to-orange-600 focus:ring-orange-100`}
        >
          <div className="w-max p-4 rounded-full bg-white fill-white bg-opacity-10">
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div>
            <strong className="text-2xl lg:text-3xl 2xl:text-4xl">
              {dashboardData.total_feedback_count}
            </strong>
            <p className="text-neutral5">Jumlah Feedback</p>
          </div>
        </Link>
        {/* Available Tokens Count */}
        <Link
          href={`${userRole.role}/token`}
          className={`flex items-center gap-4 p-6 lg:p-8 bg-gradient-to-r text-white rounded-lg hover:shadow-[rgba(0,_0,_0,_0.1)_0px_6px_10px] focus:ring-4 transition-all ease-out duration-200 from-green-700 to-green-600 focus:ring-green-100`}
        >
          <div className="w-max p-4 rounded-full bg-white fill-white bg-opacity-10">
            <svg
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div>
            <strong className="text-2xl lg:text-3xl 2xl:text-4xl">
              {dashboardData.available_tokens_count}
            </strong>
            <p className="text-neutral5">Token Aktif Belum Redeem</p>
          </div>
        </Link>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 lg:gap-3 2xl:gap-4 mt-2 lg:mt-3 2xl:mt-4">
        <Link
          href={`${userRole.role}/kelas`}
          className="h-max border-2 border-gray-200 rounded-lg p-10 flex flex-col gap-4"
        >
          {/* Title + Description */}
          <div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left rtl:text-right text-neutral3 rounded-md overflow-hidden">
              ...
              <tbody>
                {dashboardData.recent_active_classrooms.map((classData) => (
                  <tr
                    key={classData.id}
                  ...
                  >
                    <th scope="row" className="px-6 py-4 whitespace-nowrap ">
                      <div className="text-xs">
                        <p className="font-normal">{classData.lecture}</p>
                        <p className="font-medium text-neutral2">
                          {classData.name}
                        </p>
                      </div>
                    </th>
                    <td className="px-6 py-4">{classData.day}</td>
                    <td className="px-6 py-4">{classData.time_start}</td>
                    <td className="px-6 py-4">{classData.quota}</td>
                    <td className="px-6 py-4">{classData.fill}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left rtl:text-right text-neutral3 rounded-md overflow-hidden">
              ...
              <tbody>
                {dashboardData.latest_feedbacks.map((feedback) => (
                  <tr
                    key={feedback.id}
                    className="bg-white border-b hover:bg-gray-50"
                  >
                    <th scope="row" className="px-6 py-4 whitespace-nowrap">
                      <div className="text-xs">
                        <p className="font-normal">{feedback.student}</p>
                        <p className="font-medium text-neutral2">
                          {feedback.course}
                        </p>
                      </div>
