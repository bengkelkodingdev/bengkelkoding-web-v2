...
import { UpStudentData } from "@/app/interface/UserManagement";
...
const DashboardTambahPenggunaAdminPage = () => {
  ...
  const [formData, setFormData] = useState<UpStudentData>({
    id: undefined,
    identity_code: "",
    name: "",
    email: "",
  ...
  const [formDataEdit, setFormDataEdit] = useState<UpStudentData>();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (StatusForm) {
      ...
      if (name && identity_code && email) {
        try {
          // Panggil API untuk menambahkan student baru
          await updateStudent(
            {
              id: undefined,
              identity_code,
          ...
          toast.success("Berhasil menambahkan student!");
          window.location.href = "./";
        } catch (error) {
          toast.error(`Gagal menambahkan student: ${error.message}`);
        }
      } else {
  ...
  useEffect(() => {
    if (StatusForm && IdStudent) {
      findStudentData(IdStudent)
        .then((response) => {
          // Pastikan ini sesuai dengan perubahan tipe response
          if (response) {
            const studentData = response.data; // ambil dari array

            setFormDataEdit({
              id: studentData.id,
              name: studentData.name,
              identity_code: studentData.identity_code,
              email: studentData.email,
              password: studentData.password,
              is_active: studentData.is_active,
            });
          } else {
            toast.error("Data Student tidak ditemukan");
          }
        })
        .catch((error) => {
          ...
          if (error.response) {
            ...
            toast.error(
              "Gagal memuat detail Student: Tidak ada respons dari server."
            );
          } else {
            // Kesalahan lain yang mungkin terjadi
            console.error("3Error message:", error.message);
            toast.error(`Gagal memuat detail Student: ${error.message}`);
          }
        });
    }
  }, [StatusForm, IdStudent]);
      ...
      <form className="max-w-5xl flex flex-col gap-4" onSubmit={handleSubmit}>
        <div>
          ...
          <p className="text-neutral3 text-sm">
            Indentitas pengguna role mahasiswa
          </p>
        ...
        <div className="text-end">
          <Button
            type="submit"
            text={StatusForm ? "Edit Mahasiswa" : "Tambah Mahasiswa"}
          />
        </div>
      </form>
      <ToastContainer />
