...
import { ClassroomData, Presence } from "@/app/interface/Kelas";
...
const DetailKelasPageSesi = () => {
  ...
  const [classRoom, setClassRoom] = useState<Presence>();
  ...
  useEffect(() => {
    const fetchDetailSession = async () => {
      try {
        ...
        setClassRoom(presenceData);
        const getDetailClass = await getDetailClassroom(detail[0]);

        if (Array.isArray(getDetailClass.data)) {
          setdetailClassRoom(getDetailClass.data);
          console.log("Check if response.data is an object");
        } else if (
          getDetailClass.data &&
          typeof getDetailClass.data === "object"
        ) {
          console.log("adasdasdad");
          setdetailClassRoom([getDetailClass.data]); // Convert single object to array
        } else {
  ...
  if (loading) {
    return <span>Mengambil data...</span>;
  }
  ...
  return (
    <>
      {detailClassRoom.map((kelasItem) => (
        <div key={kelasItem.classroom.id} className=" flex flex-col w-full ">
